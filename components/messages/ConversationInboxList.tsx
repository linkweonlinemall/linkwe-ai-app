...
export default function ConversationInboxList({ conversations }: { conversations: InboxConversation[] }) {
  ...
  return <div className="mt-5 min-w-0">
    <div className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm">
      <label className="relative block"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-zinc-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search names or messages" className="w-full rounded-xl border border-zinc-200 bg-zinc-50 py-2.5 pl-10 pr-3 text-sm text-zinc-900 outline-none focus:border-[#D4450A] focus:bg-white focus:ring-4 focus:ring-orange-500/10" /></label>
