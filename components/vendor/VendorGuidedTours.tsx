...
export default function VendorGuidedTours() {
  ...
  return <>
    ...
    {tour && step && <div className="fixed inset-0 z-[300]" role="dialog" aria-modal="true">
      ...
      <div className={`fixed left-3 right-3 max-h-[min(52dvh,350px)] overflow-y-auto rounded-[18px] border border-white/70 bg-white shadow-[0_24px_70px_rgba(0,0,0,.34)] md:bottom-auto md:left-auto md:right-6 md:top-1/2 md:max-h-[min(72dvh,430px)] md:w-[340px] md:-translate-y-1/2 md:rounded-[22px] ${step.mobileMore ? "top-[calc(env(safe-area-inset-top,0px)+76px)]" : "bottom-[calc(74px+env(safe-area-inset-bottom,0px))]"}`}>
        <div className="h-1 bg-gradient-to-r from-[#D4450A] via-[#F27B42] to-[#E8820C]"/><div className="p-4 md:p-5">
          <div className="mb-2.5 flex items-center justify-between md:mb-4"><span className="rounded-full bg-[#FEF0EB] px-2.5 py-1 text-[9px] font-black uppercase tracking-wider text-[#D4450A] md:px-3 md:text-[10px]">{index+1} of {steps.length}</span><button type="button" onClick={() => close(false)} className="rounded-full p-1.5 text-zinc-400 hover:bg-zinc-100" aria-label="Close"><IconX className="size-4"/></button></div>
          <h3 className="text-base font-black leading-tight tracking-tight text-[#1C1C1A] md:text-xl">{step.title}</h3><p className="mt-1.5 text-xs leading-5 text-zinc-600 md:mt-2 md:text-sm md:leading-6">{step.body}</p>
          <div className="mt-3.5 h-1 overflow-hidden rounded-full bg-zinc-100 md:mt-5 md:h-1.5"><div className="h-full rounded-full bg-[#D4450A] transition-all duration-500" style={{width:`${((index+1)/steps.length)*100}%`}}/></div>
          <div className="mt-3.5 flex items-center justify-between gap-3 md:mt-5"><button type="button" disabled={index===0} onClick={() => setIndex(v=>v-1)} className="inline-flex h-9 items-center gap-1 rounded-xl px-2 text-xs font-bold text-zinc-500 disabled:invisible md:h-10 md:px-3"><IconArrowLeft className="size-4"/>Back</button>{index===steps.length-1?<button type="button" onClick={() => close(true)} className="inline-flex h-9 items-center gap-2 rounded-xl bg-[#D4450A] px-4 text-xs font-bold text-white md:h-11 md:px-5 md:text-sm"><IconCheck className="size-4"/>Finish</button>:<button type="button" onClick={() => setIndex(v=>v+1)} className="inline-flex h-9 items-center gap-2 rounded-xl bg-[#1C1C1A] px-4 text-xs font-bold text-white md:h-11 md:px-5 md:text-sm">Next<IconArrowRight className="size-4"/></button>}</div>
        </div>
      </div>
    </div>}
  </>;
