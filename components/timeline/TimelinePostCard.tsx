...
function PhotoCarousel({ postId, storeName, images, detail = false }: { postId: string; storeName: string; images: string[]; detail?: boolean }) {
  const [index, setIndex] = useState(0);
  const [lightbox, setLightbox] = useState(false);
  if (!images.length) return null;
  const src = images[index] ?? images[0]!;
  const photo = <img src={src} alt={`${storeName} post photo ${index + 1}`} className={`w-full object-cover ${detail ? "max-h-[76vh]" : "aspect-[4/5] max-h-[680px]"}`} />;
  return (
    <div className="relative overflow-hidden bg-zinc-950">
      {detail ? <button type="button" onClick={() => setLightbox(true)} className="block w-full cursor-zoom-in">{photo}</button> : <Link href={`/timeline/${postId}`} className="block">{photo}</Link>}
      {images.length > 1 ? <>
        <button type="button" onClick={() => setIndex((index - 1 + images.length) % images.length)} aria-label="Previous photo" className="absolute left-3 top-1/2 flex size-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/45 text-white shadow-lg backdrop-blur"><ChevronLeft /></button>
        <button type="button" onClick={() => setIndex((index + 1) % images.length)} aria-label="Next photo" className="absolute right-3 top-1/2 flex size-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/45 text-white shadow-lg backdrop-blur"><ChevronRight /></button>
      ...
      {lightbox ? <div role="dialog" aria-modal="true" className="fixed inset-0 z-[200] flex items-center justify-center bg-black/95 p-2 sm:p-8" onClick={() => setLightbox(false)}><button type="button" className="absolute right-4 top-4 flex size-11 items-center justify-center rounded-full bg-white/10 text-white" aria-label="Close photo" onClick={() => setLightbox(false)}><X /></button><img src={src} alt={`${storeName} post photo ${index + 1}`} className="max-h-full max-w-full object-contain" onClick={(event) => event.stopPropagation()} /></div> : null}
    </div>
  );
}
...
export default function TimelinePostCard({ initialPost, detail = false, canComment = true }: { initialPost: TimelinePost; detail?: boolean; canComment?: boolean }) {
  ...
  return <article id={post.id} className="overflow-hidden rounded-[28px] border border-white/80 bg-white shadow-[0_18px_55px_rgba(70,43,26,.12)] ring-1 ring-orange-100/50">
    ...
    <PhotoCarousel postId={post.id} storeName={post.store.name} images={post.images} detail={detail} />
