...
export default function StoreDiscoveryMap({ stores }: { stores: MapStore[] }) {
  ...
  return <section className="mt-6 overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-[0_20px_55px_rgba(28,28,26,0.12)]">
    <button type="button" onClick={() => setExpanded((v) => !v)} aria-expanded={expanded} className="flex w-full items-center gap-3 bg-gradient-to-r from-[#1C1C1A] to-[#33251f] px-4 py-4 text-left text-white"><span className="flex size-10 items-center justify-center rounded-xl bg-[#D4450A]"><MapPin className="size-5" /></span><span className="min-w-0 flex-1"><strong className="block text-sm">Explore stores on the map</strong><span className="text-xs text-white/55">{expanded ? "Tap to collapse the map" : "Tap to browse populated LinkWe regions"}</span></span><ChevronDown className={`size-5 transition ${expanded ? "rotate-180" : ""}`} /></button>
