...
export default function ReviewsList({ reviews, count, average, showProductName = false }: Props) {
        ...
        <div className="flex flex-1 flex-col gap-1.5">
          {dist.map((d) => (
            <div key={d.star} className="flex items-center gap-2">
              <span className="w-4 text-right text-xs font-semibold text-zinc-500">{d.star}</span>
              <svg viewBox="0 0 24 24" className="h-3 w-3 fill-[#E8820C] stroke-[#E8820C]" strokeWidth="1">
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
              </svg>
              <div className="flex-1 overflow-hidden rounded-full bg-zinc-100">
                <div
                  className="h-2 rounded-full bg-[#E8820C] transition-all"
                  style={{ width: `${d.pct}%` }}
                />
              </div>
              <span className="w-8 text-right text-xs text-zinc-400">{d.count}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          <div key={review.id} className="rounded-2xl border border-zinc-200 bg-white p-5">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                ...
                {review.title ? (
                  <p className="mt-1.5 text-sm font-bold text-zinc-900">{review.title}</p>
                ) : null}
              </div>
