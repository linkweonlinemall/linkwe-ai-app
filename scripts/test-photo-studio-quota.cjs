...
(async()=>{
  ...
  assert.equal(accepted.length,1); assert.equal(trialRaces.filter(r=>r.status==='rejected'&&r.reason.status===403).length,6);
  ...
  assert.equal((await getPhotoAccess(starter.id,true,new Date('2100-01-01T00:00:00Z'))).allowed,false,'free image does not reset next month');
  ...
  assert.equal((await getPhotoAccess(recreated.id,false,now)).trialRemaining,1,'sandbox does not consume live trial');
  const orphan=await fixture('orphan',false);
  await reservePhotoAttempt(orphan.id,'crashed',true,100,100,now);
  assert.equal((await getPhotoAccess(orphan.id,true,new Date(now.getTime()+601000))).trialRemaining,1,'crashed processing recovers');
  console.log('PASS Seven concurrent Starter requests allow only one; failure refunds, expiry recovery, upgrade/downgrade, store recreation and sandbox separation work');
