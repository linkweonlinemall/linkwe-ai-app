...
export default async function RegisterHubPage() {
  ...
  return (
    <>
      ...
      <div className="mx-auto w-full max-w-2xl rounded-xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="mt-8 flex w-full flex-row items-stretch gap-4">
          {cards.map((card) => (
            <Link
              key={card.href}
              className="flex min-w-0 flex-1 flex-col items-start rounded-lg border border-zinc-200 p-5 text-sm transition hover:border-zinc-400 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:border-zinc-600 dark:hover:bg-zinc-950"
              href={card.href}
            >
