...
export async function createBooking(input: {
  ...
  let coupon;
  try { coupon = await priceCoupon(service.storeId, "service", input.couponCode, [{key:`service:${service.id}`,subtotalMinor:Math.round(service.price * 100)}]); }
  catch (error) { if(error instanceof CouponError)return {error:error.message}; throw error; }
  const totalPrice = coupon ? coupon.totalMinor / 100 : service.price;
  ...
  const depositDue =
    service.depositAmount != null && service.depositAmount > 0
      ? Math.min(service.depositAmount, totalPrice)
      : null;
  const chargeAmount =
    input.paymentMethod === "online"
      ? totalPrice
...
> {
  ...
  const depositDue =
    booking.product.depositAmount != null && booking.product.depositAmount > 0
      ? Math.min(booking.product.depositAmount, booking.totalPrice)
      : null;
