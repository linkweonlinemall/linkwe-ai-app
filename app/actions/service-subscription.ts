...
export async function startServiceSubscriptionCheckout(
  ...
  let coupon;
  try{coupon=await priceCoupon(service.storeId,"service",couponCode,[{key:`service:${service.id}`,subtotalMinor:trialPriceMinor}]);}catch(error){if(error instanceof CouponError)return {ok:false,error:error.message};throw error;}
  const initialPriceMinor=coupon?.totalMinor??trialPriceMinor;
