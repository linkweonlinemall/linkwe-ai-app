...
function fields(kind: RecordKind) {
  ...
  const model = Prisma.dmmf.datamodel.models.find((m) => m.name.toLowerCase() === kind)!;
...
export async function getAdminEditableRecord(kind: RecordKind, id: string) {
  ...
  const record = kind === "product" ? await prisma.product.findUnique({ where: { id }, select }) : kind === "store" ? await prisma.store.findUnique({ where: { id }, select }) : kind === "user" ? await prisma.user.findUnique({ where: { id }, select }) : await prisma.listing.findUnique({ where: { id }, select });
  if (!record) throw new Error("Record not found.");
  return metadata.map((f) => ({ name: f.name, type: f.type, required: f.isRequired, list: f.isList, options: f.kind === "enum" ? Prisma.dmmf.datamodel.enums.find((e) => e.name === f.type)?.values.map((v) => v.name) : undefined, value: (record as Record<string, unknown>)[f.name] ?? null }));
}
export async function saveAdminEditableRecord(kind: RecordKind, id: string, values: Record<string, unknown>) {
  ...
  try {
    ...
    for (const [key, raw] of Object.entries(values)) {
      ...
      if (field.kind === "enum" && !Prisma.dmmf.datamodel.enums.find((e) => e.name === field.type)?.values.some((v) => v.name === value)) throw new Error(`Invalid ${key}.`);
    ...
    await prisma.$transaction(async (tx) => {
      if (kind === "user") {
        ...
        if (data.role && data.role !== target.role && target.role === "ADMIN") throw new Error("Administrator demotion is not available in this editor. Manage staff access separately.");
      ...
      } else if (kind === "product") await tx.product.update({ where: { id }, data: data as Prisma.ProductUpdateInput });
