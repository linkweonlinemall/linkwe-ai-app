...
export type OnboardingResult = {
  ...
  accounts: { userId: string; storeId: string; storeName: string }[];
};
...
function truthy(value: unknown) {
  return ["true", "yes", "1", "published", "active"].includes(
    String(value ?? "")
      .toLowerCase()
      .trim(),
  );
...
export async function importAdminOnboardingRows(
  rows: OnboardingRow[],
  ...
  if (!Array.isArray(rows) || rows.length === 0)
    return {
      ...result,
      ok: false,
      errors: [{ row: 0, message: "Add at least one row." }],
    };
  if (rows.length > 500)
    return {
      ...result,
      ok: false,
      errors: [{ row: 0, message: "Import up to 500 rows at a time." }],
    };

  // Rows with the same email belong to one vendor/store and may each add an item.
  const accounts = new Map<
    string,
    { firstRow: number; rows: OnboardingRow[] }
  >();
  rows.forEach((row, index) => {
    const email = clean(row.email, 254).toLowerCase();
    if (!accounts.has(email))
      accounts.set(email, { firstRow: index + 2, rows: [] });
    ...
    try {
      const existing = await prisma.user.findUnique({
        ...
        select: {
          ...
          isActive: true,
          suspended: true,
          storesOwned: { select: { id: true }, take: 1 },
        },
      });
      if (existing && (!existing.isActive || existing.suspended))
        throw new Error(
          "This vendor account is inactive or suspended. Review its access before adding a store or offerings.",
        );
      if (existing && existing.role !== "VENDOR")
        throw new Error("This email belongs to a non-vendor account.");
      const suppliedPassword =
        typeof base.password === "string" ? base.password : "";
      const generatedPassword =
        suppliedPassword || `${randomBytes(9).toString("base64url")}!Aa7`;
      if (
        generatedPassword.length < 12 ||
        Buffer.byteLength(generatedPassword, "utf8") > 72
      )
      ...
      const summary = await prisma.$transaction(
        async (tx) => {
          ...
          if (!userId) {
            const user = await tx.user.create({
              data: {
                fullName,
                email,
                phone: clean(base.phone, 30) || null,
          ...
          let madeStore = false;
          if (!storeId) {
            const collision = await tx.store.findUnique({
              where: { slug: storeBaseSlug },
              select: { id: true },
            });
            const slug = collision
              ? `${storeBaseSlug}-${randomUUID().slice(0, 6)}`
              : storeBaseSlug;
          ...
          for (const row of group.rows) {
            ...
            const stock = clean(row.stock) ? Number(row.stock) : null;
            if (!Number.isFinite(price) || price < 0)
              throw new Error(
                "Offering price must be zero or a positive number.",
              );
            if (stock != null && (!Number.isInteger(stock) || stock < 0))
              throw new Error("Stock must be a positive whole number or zero.");
            if (!["product", "service"].includes(type))
              throw new Error("itemType must be product or service.");
            const duplicate = await tx.product.findFirst({
              where: { storeId, name, isService: type === "service" },
              select: { id: true },
            });
            ...
            await tx.product.create({
              data: {
                ...
                serviceType: type === "service" ? "QUOTE" : null,
                isPublished: truthy(row.publish),
              },
            });
            items++;
          }
          await tx.notification.create({
            data: {
              ...
              body: `${fullName} · ${storeName} · ${items} item${items === 1 ? "" : "s"}`,
              linkUrl: `/dashboard/admin/users`,
            },
          });
          return { madeUser, madeStore, items, userId, storeId };
        },
