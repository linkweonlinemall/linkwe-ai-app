...
function validateSubscriptionConfiguration(input: {
  serviceType: string;
  ...
  if (input.sessionsIncluded != null && (!Number.isInteger(input.sessionsIncluded) || input.sessionsIncluded < 1 || input.sessionsIncluded > 1000)) {
    return "Sessions per cycle must be between 1 and 1,000.";
  }
  if (input.cancellationDays != null && (!Number.isInteger(input.cancellationDays) || input.cancellationDays < 0 || input.cancellationDays > 365)) {
    return "Cancellation notice must be between 0 and 365 days.";
  }
  const cycleDays = input.interval === "weekly" ? 7 : input.interval === "fortnightly" ? 14 : input.interval === "quarterly" ? 92 : 31;
  ...
  if (input.trialDays != null && (!Number.isInteger(input.trialDays) || input.trialDays < 1 || input.trialDays > 365)) {
    return "Trial length must be between 1 and 365 days.";
  }
  if (input.trialPrice != null && (!Number.isFinite(input.trialPrice) || input.trialPrice < 0 || input.trialPrice > input.price)) {
  ...
  if (input.canPause && (!Number.isInteger(input.pauseMaxWeeks) || (input.pauseMaxWeeks ?? 0) < 1 || (input.pauseMaxWeeks ?? 0) > 52)) {
