...
export default function OperationsAssistant() {
  ...
  async function execute(action: AssistantAction) {
    ...
    try { const r = await executeOperationsAction(action.token); const message=r.ok ? "Completed — the requested records were updated." : ("error" in r ? r.error : undefined) ?? "Could not complete action."; setResults(prev=>({...prev,[action.token!]:{ok:r.ok,message}})); setTurns(prev=>[...prev,{role:"assistant",content:`${action.title}: ${message}`}]); }
  ...
  return <section className="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm">
    ...
    {!turns.length && <div className="my-6 grid gap-2 sm:grid-cols-2">{["What needs attention first?","Approve all eligible pending payouts.","Show me draft stores and listings.","Which deliveries can be confirmed?"].map(q=><button key={q} onClick={()=>void ask(q)} disabled={busy} className="rounded-2xl border border-zinc-100 bg-[#F7F7F6] p-4 text-left text-sm text-zinc-600 hover:bg-orange-50">{q}</button>)}</div>}
    ...
    <form onSubmit={e=>{e.preventDefault();void ask(question);}} className="flex items-end gap-2"><textarea aria-label="Ask LinkWe assistant" maxLength={2000} rows={2} value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Publish a store, approve payouts, reply to a conversation, or update deliveries…" className="min-w-0 flex-1 resize-none rounded-2xl border border-zinc-200 p-3 text-sm outline-none focus:border-orange-400"/><button disabled={busy || !question.trim()} aria-label="Send request" className="min-h-12 rounded-xl bg-[#D4450A] px-4 text-white disabled:opacity-40"><ArrowUp size={20}/></button></form><p className="mt-3 text-[11px] leading-5 text-zinc-400">Every write is presented for confirmation. Previews expire after 10 minutes; order previews cannot overwrite newer changes. CSF bookings remain in their merchant portal.</p></div>
  </section>;
}
