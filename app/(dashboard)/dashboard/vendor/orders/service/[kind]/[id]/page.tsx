...
export default async function VendorServiceOrderDetailPage({ params }: Props) {
  ...
  if (kind === "booking") {
    ...
    manageHref=`/dashboard/vendor/bookings?booking=${encodeURIComponent(id)}`;manageLabel="Manage booking";customerNotes=booking.customerNotes;vendorNotes=booking.vendorNotes;
    ...
    orderKind=request.requestType==="QUOTE"?"quote":"request";prefix=orderKind==="quote"?"QT":"RQ";manageHref=`/dashboard/vendor/requests?request=${encodeURIComponent(id)}#request-${encodeURIComponent(id)}`;manageLabel=orderKind==="quote"?"Manage quote":"Manage request";
    ...
    manageHref="/dashboard/vendor/subscribers";manageLabel="Manage subscriber";
  ...
  return <div className={s.page}>
    ...
    <div className={s.detailGrid}>
      ...
      <aside className={s.detailAside}>
        ...
        <section className={s.panel}><h2>Your service tools</h2><p className={s.caption}>Manage appointments, respond to requests and keep your subscribers up to date.</p><div style={{display:"grid",gap:9,marginTop:17}}>{[{label:"Bookings",path:"bookings"},{label:"Requests & quotes",path:"requests"},{label:"Subscribers",path:"subscribers"}].map(tool=><Link className={s.secondary} key={tool.path} href={`/dashboard/vendor/${tool.path}`}>{tool.label}<ArrowUpRight size={15}/></Link>)}</div></section>
      </aside>
    </div>
  </div>;
}
