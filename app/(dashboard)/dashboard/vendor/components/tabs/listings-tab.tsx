...
export default function ListingsTab({ listings }: Props) {
  return (
    <div className="flex flex-col gap-6">
      <div>
        <Link
          className="inline-flex h-10 items-center justify-center rounded-lg bg-[#D4450A] px-4 text-sm font-medium text-white hover:bg-[#B83A08]"
      ...
      <section className="rounded-lg border border-zinc-200 bg-zinc-50 p-6">
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-wide text-zinc-500">Your listings</h2>
        {listings.length === 0 ? (
          <p className="mt-3 text-sm text-zinc-600">No listings yet. Create one above.</p>
        ) : (
            ...
            {listings.map((listing) => (
              ...
              >
                ...
                <div className="flex shrink-0 flex-wrap gap-2 sm:self-center">
                  <Link
                    className="inline-flex h-9 items-center justify-center rounded-lg border border-zinc-300 px-3 text-sm font-medium text-zinc-900 hover:bg-white"
