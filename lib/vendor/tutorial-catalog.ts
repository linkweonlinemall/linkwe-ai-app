...
export const tutorialCatalog = {
  ...
  partners: {
    label: "Partners", description: "Send, review, cancel and manage vendor partnerships.", category: "Business operations", duration: "4 min",
    steps: [routeStep("/dashboard/vendor/partners", "Vendor partnerships", "Use partnerships for relevant business collaboration—not unsolicited mass requests."),
      { route: "/dashboard/vendor/partners", selector: '[data-tour="partner-search"]', title: "Find a partner", body: "Search and review the vendor’s public store before sending a request." },
      { route: "/dashboard/vendor/partners", selector: '[data-tour="partner-send"]', title: "Send a clear request", body: "Explain the business purpose and what collaboration you are proposing." },
      { route: "/dashboard/vendor/partners", selector: '[data-tour="partner-incoming"]', title: "Incoming requests", body: "Accept only recognised, relevant vendors. Decline requests that do not fit or appear unsafe." },
      { route: "/dashboard/vendor/partners", selector: '[data-tour="partner-outgoing"]', title: "Outgoing requests", body: "Cancel your pending request when plans change so it does not remain open indefinitely." },
      { route: "/dashboard/vendor/partners", selector: '[data-tour="partner-active"]', title: "Active partners", body: "Review active relationships and remove partnerships that are no longer valid." }],
  },
  shipping: {
    label: "Shipping & pickup", description: "Configure fulfilment methods, zones and delivery pricing.", category: "Business operations", duration: "7 min",
    steps: [routeStep("/dashboard/vendor/shipping", "Shipping setup", "Store-level shipping determines the delivery choices available to eligible physical products."),
      { route: "/dashboard/vendor/shipping", selector: '[data-tour="shipping-methods"]', title: "Fulfilment methods", body: "Choose vendor delivery, LinkWe delivery and local pickup according to what the business can reliably fulfil." },
      { route: "/dashboard/vendor/shipping", selector: '[data-tour="shipping-home-zone"]', title: "Home zone", body: "Confirm the store’s home delivery zone because suggested rates and coverage depend on it." },
      { route: "/dashboard/vendor/shipping", selector: '[data-tour="shipping-zones"]', title: "Delivery zones", body: "For each zone, set the rate and whether you deliver there. Do not enable regions you cannot serve." },
      { route: "/dashboard/vendor/shipping", selector: '[data-tour="shipping-linkwe-fallback"]', title: "Let LinkWe deliver", body: "Enable LinkWe delivery for a zone when you are not self-delivering there and LinkWe coverage is available." },
      { route: "/dashboard/vendor/shipping", selector: '[data-tour="shipping-pickup"]', title: "Local pickup", body: "Enable only with a safe, reliable collection point. Give customers clear pickup readiness and collection instructions." },
      { route: "/dashboard/vendor/shipping", selector: '[data-tour="shipping-save"]', title: "Save rates", body: "Save every change, then test cart delivery totals using multiple regions and pickup." },
      { title: "Product-level check", body: "A product must also allow delivery or pickup. Enter accurate packaged weight/dimensions for LinkWe delivery calculations." }],
  },
  staff: {
    ...
    steps: [routeStep("/dashboard/vendor/staff", "Availability centre", "Availability controls when customers can book services and which staff member can perform the work."),
      { route: "/dashboard/vendor/staff", selector: '[data-tour="availability-tabs"]', title: "Staff and schedule views", body: "Use the correct view to manage people, service assignments and time availability." },
      { route: "/dashboard/vendor/staff", selector: '[data-tour="staff-list"]', title: "Staff members", body: "Add only active workers, use recognisable names and keep contact/role information current." },
      { route: "/dashboard/vendor/staff", selector: '[data-tour="staff-services"]', title: "Service assignments", body: "Assign only services the staff member can deliver. Unassigned services may have no bookable provider." },
      { route: "/dashboard/vendor/staff", selector: '[data-tour="availability-calendar"]', title: "Working schedule", body: "Set regular days and time windows. Include realistic service duration and transition time." },
      { route: "/dashboard/vendor/staff", selector: '[data-tour="availability-blocks"]', title: "Blocked time", body: "Add leave, closures and one-off unavailable periods before customers can book them." },
      { route: "/dashboard/vendor/staff", selector: '[data-tour="availability-save"]', title: "Save and test", body: "Save, then open the public service and verify the expected slots appear without conflicts." }],
  },
  finance: {
  ...
  settings: {
    ...
    steps: [routeStep("/dashboard/vendor/settings", "Vendor settings", "Settings controls personal account information and account access—not public store content."),
      ...
      { route: "/dashboard/vendor/settings", selector: '[data-tour="settings-security"]', title: "Security", body: "Use a strong unique password, protect email access and sign out on shared devices." },
      { route: "/dashboard/vendor/settings", selector: '[data-tour="settings-notifications"]', title: "Notifications", body: "Keep essential order, booking, request, message and payment notifications enabled so customer work is not missed." },
      { route: "/dashboard/vendor/settings", selector: '[data-tour="settings-signout"]', title: "Sign out", body: "Sign out when using a shared or public device. This does not delete the store." }],
  },
  rex: {
